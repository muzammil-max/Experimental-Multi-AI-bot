...
const modelsData = {
  "DallE-XL": {
    ...
    triggerId: 6,
    useSize: false,
    dataPattern: (prompt, randomDigit, width, height) => [
      prompt,
      nevPrompt,
      true,
      randomDigit,
  ...
  Anime: {
    ...
    triggerId: 49,
    useSize: true,
    dataPattern: (prompt, randomDigit, size) => [
      prompt,
      nevPrompt,
      randomDigit,
      1024,
...
function getResolution(resolution) {
  switch (resolution) {
    case "Square":
      return { width: 1024, height: 1024, size: "1024 x 1024" };
    case "Wide":
      return { width: 1280, height: 768, size: "1344 x 768" };
    case "Portrait":
      return { width: 768, height: 1280, size: "832 x 1216" };
    default:
...
function generateImage(prompt, resolution, model) {
  return new Promise((resolve, reject) => {
    try {
      ...
      const { url, fnIndex, triggerId, useSize, dataPattern } =
        modelsData[model];
      const randomDigit = generateRandomDigits();
      const session_hash = generateSessionHash();
        ...
        .then((responseFirst) => {
          ...
          eventSource.onmessage = (event) => {
            ...
            if (data.msg === "process_completed") {
              eventSource.close();
              const full_url =
                data?.output?.data?.[0]?.[0]?.image?.url ||
                data?.output?.data?.[0]?.url ||
                data?.output?.data?.[1]?.url;
              if (full_url) {
...
const imgModels = [...Object.keys(modelsData), "Playground", "DallE-3"];

const imageModelFunctions = {
  ...Object.fromEntries(
    Object.keys(modelsData).map((model) => [model, generateImage])
  ),
