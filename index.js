...
const FILE_PATHS = {
  ...
  userPreferredImageModel: path.join(
    CONFIG_DIR,
    "user_preferred_image_model.json"
  ),
  userPreferredImageResolution: path.join(
    CONFIG_DIR,
    "user_preferred_image_resolution.json"
  ),
  userPreferredImagePromptEnhancement: path.join(
    CONFIG_DIR,
    "user_preferred_image_prompt_enhancement.json"
  ),
  userPreferredSpeechModel: path.join(
    CONFIG_DIR,
    "user_preferred_speech_model.json"
  ),
  ...
  try {
    if (!workInDMs && interaction.channel.type === ChannelType.DM) {
      const embed = new EmbedBuilder()
        ...
        .setDescription("CrackGPT can't send any DMs to this person.");
      return interaction.reply({ embeds: [embed], ephemeral: true });
    }
    if (interaction.guild) {
      initializeBlacklistForGuild(interaction.guild.id);
      if (
        ...
        const embed = new EmbedBuilder()
          .setColor(hexColour)
          .setTitle("Blacklisted")
          .setDescription("You are blacklisted and cannot use this option.");
        return interaction.reply({ embeds: [embed], ephemeral: true });
      }
    }
...
async function handleSpeechCommand(interaction) {
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(`CrackGPT is generating your speech, please wait... 💽`);
    await interaction.reply({ embeds: [embed], ephemeral: true });
    const userId = interaction.user.id;
    ...
    if (outputUrl && outputUrl !== "Output URL is not available.") {
      ...
      const embed = new EmbedBuilder()
        ...
        .setTitle("Error")
        .setDescription(
          `Sorry, something went wrong, or the output URL is not available.\n> **Text:**\n\`\`\`\n${
    ...
    try {
      const embed = new EmbedBuilder()
        ...
        .setTitle("Error")
        .setDescription(
          `Sorry, something went wrong and the output is not available.\n> **Text:**\n\`\`\`\n${
            text.length > 3900 ? text.substring(0, 3900) + "..." : text
          }\n\`\`\``
        );
      const messageReference = await interaction.channel.send({
    ...
    if (isGuild) {
      embed.setFooter({
        ...
        iconURL:
          ...
          "https://media.licdn.com/dms/image/sync/v2/D4D27AQENE41PVzrNEQ/articleshare-shrink_800/articleshare-shrink_800/0/1713572266644?e=2147483647&v=beta&t=gBrvCvLH9Op1k4MCa6aQ0OLSJzE8OvxFVZeHM1tWxPc",
      });
    }
...
async function genimg(prompt, message) {
  ...
  try {
    ...
    const attachment = new AttachmentBuilder(imageUrl, {
      name: `generated-image${imageExtension}`,
    });
    const embed = new EmbedBuilder()
      .setColor(hexColour)
      .setAuthor({
      ...
      .addFields(
        ...
        //
        {
          ...
          value: `\`${
            enhancedPrompt !== "Disabled" ? "Enabled" : "Disabled"
          }\``,
          inline: true,
        }
      )
    ...
    try {
      const embed = new EmbedBuilder()
        ...
        .setTitle("Error")
        .setDescription(
          `Sorry, could not generate the image. Please try again later.\n> **Prompt:**\n\`\`\`\n${
            prompt.length > 3900 ? prompt.substring(0, 3900) + "..." : prompt
          }\n\`\`\``
        );
...
async function genimgslash(prompt, modelInput, interaction) {
  ...
  const preferredModel =
    modelInput || userPreferredImageModel[userId] || defaultImgModel;
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `Generating your image with \`${preferredModel}\`, please wait... 🖌️`
    );
  await interaction.reply({ embeds: [embed], ephemeral: true });
...
async function handleImageGenerationError(interaction, prompt) {
  try {
    const embed = new EmbedBuilder()
      ...
      .setTitle("Error")
      .setDescription(
        `Sorry, the image could not be generated. Please try again later.\n> **Prompt:**\n\`\`\`\n${
          prompt.length > 3900 ? prompt.substring(0, 3900) + "..." : prompt
        }\n\`\`\``
      );
  ...
  try {
    ...
    if (isGuild) {
      embed.setFooter({
        ...
        iconURL:
          interaction.guild.iconURL() ||
          "https://upload.wikimedia.org/wikipedia/commons/thumb/f/f0/Google_Bard_logo.svg/2048px-Google_Bard_logo.svg.png",
      });
    }
...
async function changeImageResolution(interaction) {
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        "Select the resolution you want to use for image generation."
      );
...
async function handleImageSelectResolution(interaction, resolution) {
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `Image Generation Resolution Selected: \`${resolution}\``
      );
...
async function setCustomPersonality(interaction) {
  ...
  const input = new TextInputBuilder()
    ...
    .setLabel(
      "If you want to alter CrackGPT personanlity , you can change it here"
    )
    .setStyle(TextInputStyle.Paragraph)
...
async function downloadMessage(interaction) {
  try {
    ...
    const updatedEmbed = EmbedBuilder.from(response.embeds[0]).setDescription(
...
async function downloadConversation(interaction) {
  try {
    ...
    for (let i = 0; i < conversationHistory.length; i++) {
      const role =
        conversationHistory[i].role === "user" ? "[User]" : "[Model]";
      const content = conversationHistory[i].parts
        .map((c) => c.text)
        .join("\n");
...
async function toggleServerWideChatHistory(interaction) {
  try {
    ...
    const statusMessage = `Server-wide Chat History is now \`${
      serverSettings[serverId].serverChatHistory ? "enabled" : "disabled"
    }\``;
...
async function toggleServerPersonality(interaction) {
  try {
    ...
    const statusMessage = `Server-wide Personality is now \`${
      serverSettings[serverId].customServerPersonality ? "enabled" : "disabled"
    }\``;

    const embed = new EmbedBuilder()
      .setColor(
        serverSettings[serverId].customServerPersonality ? 0x00ff00 : 0xff0000
      )
...
async function toggleServerResponsePreference(interaction) {
  try {
    ...
    const statusMessage = `Server-wide Response Following is now \`${
      serverSettings[serverId].serverResponsePreference ? "enabled" : "disabled"
    }\``;
...
async function toggleSettingSaveButton(interaction) {
  try {
    ...
    const statusMessage = `Server-wide "Settings and Save Button" is now \`${
      serverSettings[serverId].settingsSaveButton ? "enabled" : "disabled"
    }\``;

    const embed = new EmbedBuilder()
      .setColor(
        serverSettings[serverId].settingsSaveButton ? 0x00ff00 : 0xff0000
      )
...
async function downloadServerConversation(interaction) {
  try {
    ...
    const tempFileName = path.join(
      __dirname,
      `${guildId}_server_conversation.txt`
    );
    fs.writeFileSync(tempFileName, conversationText, "utf8");
...
async function showSettings(interaction, edit = false) {
  try {
    ...
    const mainButtons = [
      ...
      {
        customId: "image-settings",
        label: "Image Settings",
        emoji: "🖼️",
        style: ButtonStyle.Secondary,
      },
      {
        customId: "speech-settings",
        label: "Speech Settings",
        emoji: "🎤",
        style: ButtonStyle.Secondary,
      },
      {
        customId: "music-settings",
    ...
    "general-settings": [
      {
        ...
        label: `Always Respond: ${
          activeUsersInChannels[channelId][userId] ? "ON" : "OFF"
        }`,
        emoji: "↩️",
        style: ButtonStyle.Secondary,
      },
...
async function handleModelResponse(
  ...
  let botMessage;
  const loadingGifEmoji = ":loading_gif:"; // Replace with your server's custom animated emoji name
  ...
  if (!initialBotMessage) {
    ...
    try {
      botMessage = await originalMessage.reply({
        content:
          "**CrackGPT is generating your response, please wait...**<a:loading:1320078156371198064>", // Replace with your animated emoji ID
      });
    } catch (error) {
      console.error(error);
    }
    ...
    try {
      botMessage.edit({
        content: `CrackGPT is generating your response... **<a:loading:1320078156371198064>`,
      });
    } catch (error) {
      console.error(error);
    }
  ...
  async function generateGraph(data, labels, type = "line", title = "Graph") {
    ...
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(50, 550); // x-axis
    ctx.lineTo(750, 550);
    ctx.moveTo(50, 550); // y-axis
    ctx.lineTo(50, 50);
    ctx.stroke();
    ...
    const scaledData = data.map((value) =>
      math.map(value, minData, maxData, 500, 100)
    );
    ...
    if (type === "line") {
      ...
      scaledData.forEach((y, i) => {
        const x = math.map(i, 0, data.length - 1, 100, 700);
        if (i === 0) {
          ctx.moveTo(x, y);
        } else {
          ctx.lineTo(x, y);
        }
        ctx.arc(x, y, 5, 0, Math.PI * 2); // Add point
      });
      ctx.stroke();
    } else if (type === "bar") {
      scaledData.forEach((y, i) => {
        const x = math.map(i, 0, data.length - 1, 100, 700);
        ctx.fillRect(x - 10, y, 20, 550 - y);
      });
    }

    // Labels
    labels.forEach((label, i) => {
      const x = math.map(i, 0, labels.length - 1, 100, 700);
      ctx.fillText(label, x, 570);
    });

    // Return graph as buffer
    return canvas.toBuffer("image/png");

    async function handleGraphCommand(data, labels, type, title) {
      try {
        const graphBuffer = await generateGraph(data, labels, type, title);
        await originalMessage.channel.send({
          content: "Here is your graph:",
          files: [{ attachment: graphBuffer, name: "graph.png" }],
        });
      } catch (error) {
      ...
      // Example usage for generating a graph
      if (parts.command === "generateGraph") {
        const { data, labels, type, title } = parts.args; // Assuming input parsing is handled
    ...
    try {
          ...
          if (finalResponse.length > maxCharacterLimit) {
            if (!isLargeResponse) {
              ...
              const embed = new EmbedBuilder()
                ...
                .setDescription(
                  "The response is too large and will continue in subsequent messages."
                );

              botMessage.edit({ embeds: [embed] });
            }

            const currentResponse = finalResponse.slice(0, maxCharacterLimit);
            finalResponse = finalResponse.slice(maxCharacterLimit);

            // Send well-formatted response chunk
            const formattedResponse = formatResponse(currentResponse);
            await originalMessage.channel.send(formattedResponse);
          }
        }
        newHistory.push({
          role: "assistant",
      ...
      if (isLargeResponse) {
        if (finalResponse.length > 0) {
          const responseChunks = finalResponse.match(
            new RegExp(`.{1,${maxCharacterLimit}}`, "g")
          );
          for (const chunk of responseChunks) {
            const formattedResponse = formatResponse(chunk);
            await originalMessage.channel.send(formattedResponse);
          }
        }
      } else {
...
${error.message}`);
  ...
  // Function to format the response for Discord
    ...
    return response
      ...
      .map((line) => {
        ...
        if (line.startsWith("#")) {
          ...
          // Return plain text with trimmed whitespace
          return line.trim();
        }
      })
      .join("\n");
